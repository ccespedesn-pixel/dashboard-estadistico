...
function descargarDoc(endpoint, nombre) {
  const a = document.createElement('a');
  a.href = apiUrl(endpoint);
  a.download = nombre;
...
export default function DossierCalidad() {
  ...
  useEffect(() => {
    fetch(apiUrl('/api/dossier/listado'))
      .then((r) => r.json())
  ...
  const handleDescargar = async (doc) => {
    ...
    try {
      const res = await fetch(apiUrl(doc.endpoint));
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-6 text-white">
        ...
        <p className="text-sm text-slate-300">Documentación técnica y administrativa del proyecto de Seguridad Ciudadana. Haz clic en cada documento para descargarlo en formato Word (.docx).</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {docs.map((doc) => (
            ...
            >
              ...
              <div className="text-xs text-slate-400 font-mono mb-1">Capítulo {doc.id}</div>
